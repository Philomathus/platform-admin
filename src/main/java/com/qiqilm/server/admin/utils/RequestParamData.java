...
public class RequestParamData {
    ...
    private static final String RE_AG_PLAY_DETAIL_RECORD_S1    = "cagent=%s&startdate=%s&enddate=%s&gametype=%s&gamecode=%s"
            + "&page=1&perpage=100&key=%s";
    ...
    public static String requestAGPlayBetDetail( MemberGameData memberGameData, String url ) throws Exception {
        ...
        String key             = DigestUtils.md5Hex(
                lineCode + stringStartDate + stringEndDate + memberGameData.getKindId() + memberGameData.getGameRound() + "1100"
                        + "5F14237EE2A67EF102203A4C97603BC5" );
        String s1 = String.format( RE_AG_PLAY_DETAIL_RECORD_S1, lineCode, stringStartDate, stringEndDate,
                memberGameData.getKindId(), memberGameData.getGameRound(), key );
        String param = s1;
        //先写死，后续在处理 GY9
    ...
    //沙巴体育-投注记录
    public static List<Map<String, String>> requestSbSportBetRecord( GamePlatform gamePlatform, MemberGameData memberGameData,
    ...
    //沙巴体育-投注详情
    public static AjaxResult requestSbSportBetDetail( MemberGameData memberGameData, GamePlatform gamePlatform,
                                                      RestTemplate restTemplate ) throws Exception {
    ...
    //BBIN体育投注列表
    public static List<Map<String, String>> requestBBINSportBetRecord( MemberGameData memberGameData,
        ...
        builder.append( "website=" ).append( gamePlatform.getDes() ).append( "&action=" ).append( "BetTime" )
               .append( "&uppername=" ).append( gamePlatform.getAgent() ).append( "&date=" ).append( date )
               .append( "&starttime=" ).append( beginTime ).append( "&endtime=" ).append( endTime ).append( "&key=" ).append( a )
               .append( md5 ).append( c );
    ...
    public static String requestBBINSportBetDetail( MemberGameData memberGameData, GamePlatform gamePlatform ) throws Exception {
        ...
        String s1 = String.format( RE_BBIN_SPORT_DETAIL_RECORD_S1, des, account, memberGameData.getGameRound(), key );
        String param  = s1;
        String apiUrl = gamePlatform.getApiUrl();
    ...
    public static String requestMTBetRecord( MemberGameData memberGameData, GamePlatform gamePlatform ) throws Exception {
        ...
        String s1         = String.format( RE_MT_DETAIL_RECORD_S1, merchantId, code, Base64.getEncoder().encodeToString( JsonUtil
                .object2Json( data ).getBytes() ) );
        String apiUrl     = gamePlatform.getRecordUrl();
        String getURL     = apiUrl + s1;
        log.info( "美天棋牌-对局详情-请求参数：{}", getURL );
        return PostData.post( getURL, null );
    }
    ...
    public static String getBetDetailURLByKXOrKY( MemberGameData memberGameData, GamePlatform gamePlatform ) throws Exception {
        ...
        String key     = DigestUtils.md5Hex( agent + nowTime.getTime() + md5 );
        String s1      = String.format( RE_KY_DETAIL_RECORD_S1, agent, nowTime.getTime(), "{0}", key );
        String s2      = String.format( RE_KY_DETAIL_RECORD_S3, "10", memberGameData.getKindId(), memberGameData.getRecordId(),
                memberGameData.getAccount() );
        String s3      = Encrypt.AESEncrypt( s2, gamePlatform.getDes() );
        String param   = s1.replace( "{0}", s3 );
        String apiUrl  = gamePlatform.getRecordUrl();
        return apiUrl.concat( "?" ).concat( param );
    ...
    public static String getBetDetailURLByXSJ( MemberGameData memberGameData, GamePlatform gamePlatform ) throws Exception {
        ...
        String key     = DigestUtils.md5Hex( agent + nowTime.getTime() + md5 );
        String s1      = String.format( RE_XSJ_DETAIL_RECORD_S1, agent, nowTime.getTime(), "{0}", key );
        String s2 = String.format( RE_XSJ_DETAIL_RECORD_S3, "19", memberGameData.getGameUserNo(), memberGameData.getRecordId(),
                memberGameData.getAccount(), memberGameData.getServerId() );
        log.info( "新世界请求参数:{}", s2 );
        String s3     = Encrypt.AESEncrypt( s2, gamePlatform.getDes() );
        String param  = s1.replace( "{0}", s3 );
        String apiUrl = gamePlatform.getRecordUrl();
