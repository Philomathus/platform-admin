...
public class TeLunSuPayAgentProcessor extends AbstractPayAgent {
	...
	public boolean orderPay( MemberWithdrawLog withdrawLog, PayAgentPlatform payAgentPlatform, ReqPayAgent reqPayAgent ) throws Exception {
		...
		log.info( "特仑苏代付下单结果" + JsonUtil.object2Json(resultMap) );
		if ( !CollectionUtils.isEmpty( resultMap ) ) {
			if ( "success".equals( resultMap.getOrDefault( "msg", "" ).toString() ) ) {
				log.warn( "特仑苏代付订单提交成功 - result:{}", JsonUtil.object2Json( resultMap ) );
				return true;
			}else{
		...
		log.warn( "特仑苏代付订单提交失败 - orderNo:{}", withdrawLog.getOrderNo());
		return false;
	}
	...
	public String queryOrderPay( PayAgentLog payAgentLog ) throws Exception {
		...
		log.warn( "特仑苏查询结果:" + JsonUtil.object2Json(resultMap) );
		if ( !CollectionUtils.isEmpty( resultMap ) ) {
