...
public class YiXin2PayAgentProcessor extends AbstractPayAgent {
    ...
    public boolean orderPay(MemberWithdrawLog withdrawLog, PayAgentPlatform payAgentPlatform, ReqPayAgent reqPayAgent) throws Exception {
        ...
        try {
            String result = restTemplate.postForObject(payAgentPlatform.getPayOrderAddr(), httpEntity, String.class);
            log.info("亿信代付申请返回结果:{}",JsonUtil.object2Json(result));
    ...
    public void queryOrderPay(PayAgentLog payAgentLog) throws Exception {
        ...
        try {
            String result = restTemplate.postForObject(payAgentPlatform.getPayOrderAddr(), httpEntity, String.class);
            log.info("亿信代付查询返回结果:{}",JsonUtil.object2Json(result));
            if (Strings.isNotBlank(result) ) {
                Map resultMap = JsonUtil.json2Map( result );
                if (!CollectionUtils.isEmpty(resultMap)) {
                    if ("200".equals(resultMap.getOrDefault("code", "").toString())) {
