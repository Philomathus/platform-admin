...
@Repository( value = ConstantsPayAgent.XIONG_MAO + "PayAgentProcessor" )
@Log4j2
public class XiongMaoPayAgentProcessor extends AbstractPayAgent  {
	@Override
	public boolean orderPay(MemberWithdrawLog withdrawLog, PayAgentPlatform payAgentPlatform, ReqPayAgent reqPayAgent ) throws Exception {
		Map<String, String> dataMap = new LinkedHashMap<>();
		dataMap.put( "pid", payAgentPlatform.getMerId() );
		dataMap.put( "money",withdrawLog.getWithdrawMoney().setScale( 2, RoundingMode.HALF_UP ).toString()  );
		dataMap.put( "sn", withdrawLog.getOrderNo() );
		dataMap.put( "bc_type", "bank" );
		dataMap.put( "bc_name", withdrawLog.getBankName() );
		dataMap.put( "bc_num", withdrawLog.getBankAccount() );
		dataMap.put( "bc_user", withdrawLog.getBankUserName() );

		String signMd5 = RSACoder.decryptByPrivateKey( payAgentPlatform.getSignMd5(), AuthUtil.getSecurityKeyStr(
				"secretkey/payAgentPrivateKey" ) );
		dataMap.put( "notify_url", sysConfigCacheUtil.getConf( "payAgentNotifyUrl" ) + ConstantsPayAgent.TIAN_TIAN );
