...
@Repository( value = ConstantsPayAgent.HUOFENGHUANG + "PayAgentProcessor" )
@Log4j2
public class HuoFengHuangPayAgentProcessor extends AbstractPayAgent {
	@Override
	public boolean orderPay( MemberWithdrawLog withdrawLog, PayAgentPlatform payAgentPlatform, ReqPayAgent reqPayAgent ) throws Exception {
		...
		if ( !CollectionUtils.isEmpty( resultMap ) ) {
			if ( "success".equals( resultMap.getOrDefault( "status", "" ).toString() ) ) {
				log.info( "火凤凰代付订单提交成功 - result:{}", JsonUtil.object2Json( resultMap ) );
				return true;
			} else {
		...
		log.warn( "火凤凰代付订单提交失败 - orderNo:{}", withdrawLog.getOrderNo() );
		return false;
	}
	...
	public String queryOrderPay( PayAgentLog payAgentLog ) throws Exception {
		...
		try {
			...
			log.info( "火凤凰代付查询结果 - result:{}", JsonUtil.object2Json( resultMap ) );
			if ( !CollectionUtils.isEmpty( resultMap ) ) {
		...
		return "火凤凰代付查询失败,订单号:" + withdrawLog.getOrderNo();
	}
}
