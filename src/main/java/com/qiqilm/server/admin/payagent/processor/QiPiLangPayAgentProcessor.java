...
public class QiPiLangPayAgentProcessor extends AbstractPayAgent {
    ...
    public boolean orderPay(MemberWithdrawLog withdrawLog, PayAgentPlatform payAgentPlatform, ReqPayAgent reqPayAgent) throws Exception {
        ...
        if (!CollectionUtils.isEmpty(resultMap)) {
            ...
            if ("0000".equals(code)) {
                log.info("七匹狼代付订单提交成功 - result:{}", JsonUtil.object2Json(resultMap));
                return true;
            } else {
        ...
        log.warn("七匹狼代付订单提交失败 - orderNo:{}", withdrawLog.getOrderNo());
        return false;
    }
    ...
    public String queryOrderPay(PayAgentLog payAgentLog) throws Exception {
        ...
        try {
            ...
            log.info("七匹狼代付查询结果- result:{}", JsonUtil.object2Json(resultMap));
            if (!CollectionUtils.isEmpty(resultMap)) {
                String code = resultMap.getOrDefault("code", "").toString();
                if ("1".equals(code)) {
