...
@Repository ( value = ConstantsPayAgent.XIN_HE_PAY + "PayAgentProcessor" )
@Log4j2
public class XinHePayAgentProcessor extends AbstractPayAgent {
    ...
    public String callbackPay( PayAgentPlatform payAgentPlatform, Map<String, Object> requestMap, String realIp ) throws Exception {
        ...
        if ( sign.equalsIgnoreCase( signStr ) ) {
            ...
            payAgentService.processOrderPay( withdrawLog, payAgentLog, requestMap.getOrDefault( "sysorderid", "" )
        ...
        try {
            ...
            if ( !CollectionUtils.isEmpty( resultMap ) ) {
                ...
                if ( "200".equals( return_code ) ) {
                    ...
                    if ( Arrays.asList( "0", "1", "2", "3", "4", "5", "6", "7" ).contains( trade_state ) ) {
                        int status      = 0;
                        int orderStatus = 0;
                        if ( "4".equals( trade_state ) ) {
                            status = 6;
                            orderStatus = 1;
                        } else if ( Arrays.asList( "3", "4" ).contains( trade_state ) ) {
                            status = 5;
                            orderStatus = 2;
                        }
