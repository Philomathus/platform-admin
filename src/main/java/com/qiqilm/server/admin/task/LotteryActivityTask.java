...
public class LotteryActivityTask {
	...
	@Scheduled(cron="0 10 20 * * ?")// 每天16:00点执行一次
	...
	private boolean updateLotteryTimes(String userId, Integer times, BigDecimal rechargeMoney){
		...
		if ( lotteryDiceUser == null) {
			...
			return lotteryDiceUserMapper.insertLotteryDiceUser(diceUser)>0;
		}
		lotteryDiceUser.setTimes(times);
		lotteryDiceUser.setRechargeMoney(rechargeMoney);
		return lotteryDiceUserMapper.updateLotteryDiceUser(lotteryDiceUser)>0;
	}
