...
public class SysUserServiceImpl implements ISysUserService {
    ...
    @Override
    public String selectGoogleAuthKeyByUserName( String userName ) {
        return userMapper.selectGoogleAuthKeyByUserName( userName );
    }
    ...
    public AjaxResult updateMobile( String newMobile, String oldMobile, String memberId ) {
        ...
        int countHistory = memberInfoMapper.countHistoryByPhone( newMobile );
        if ( count > 0 || countHistory > 0 ) {
            return AjaxResult.error( "手机号已存在" );
        }
        MemberInfo memberInfo = memberInfoMapper.selectMemberInfoById( memberId );
        if ( memberInfo == null ) {
            memberInfo = memberInfoMapper.selectMemberInfoHistoryById( memberId );
            if ( memberInfo == null ) {
                return AjaxResult.error( "会员不存在" );
            }
            MemberInfo update = new MemberInfo();
            update.setId( memberId );
            update.setPhone( newMobile );
            memberInfoMapper.updateMemberInfoHistory( update );
        } else {
            MemberInfo update = new MemberInfo();
            update.setId( memberId );
            update.setPhone( newMobile );
