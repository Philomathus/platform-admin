...
public class PayAgentRechargeRecordServiceImpl implements IPayAgentRechargeRecordService {
	...
	public AjaxResult deposit( PayAgentRechargeRecord dto )  {
		...
		PayAgentRechargeAccount payAgentRechargeAccount=payAgentRechargeAccountMapper.selectPayAgentRechargeAccount(rechargeAcount);
		if (payAgentRechargeAccount==null){
			return AjaxResult.error("代充账号不存在");
		}
		...
		int a=payAgentRechargeRecordMapper.insertPayAgentRechargeRecord( Record );
		if (a==0){
			throw new BusinessException("插入代充数据失败");
		}
		//加钱，payAgentRechargeAccount和PayAgentRechargeTrade
		int num=payAgentRechargeRecordMapper.updateByBalanceAmount( rechargeAcount, BalanceAmount );
		if(num==0){
			throw new BusinessException("修改金额失败");
		}
		...
		int number=payAgentRechargeTradeLogMapper.insertPayAgentRechargeTradeLog( payAgentRechargeTradeLog );
		if (number==0){
			throw new BusinessException("插入代充数据失败");
		}
	...
	public AjaxResult proposed( PayAgentRechargeRecord dto )  {
		...
		PayAgentRechargeAccount payAgentRechargeAccount=payAgentRechargeAccountMapper.selectPayAgentRechargeAccount(dto.getRechargeAcount());
		if (payAgentRechargeAccount==null){
			return AjaxResult.error("提出账号不存在");
		}
		//代充人表減钱
		...
		int b=payAgentRechargeRecordMapper.insertPayAgentRechargeRecord( payAgentRechargeRecord );
		if (b==0){
			throw new BusinessException("插入代充数据失败");
		}
		...
		int num=payAgentRechargeTradeLogMapper.insertPayAgentRechargeTradeLog( payAgentRechargeTradeLog );
		if ( num == 0 ) {
