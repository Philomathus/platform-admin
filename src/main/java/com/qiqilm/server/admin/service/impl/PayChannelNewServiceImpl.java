    ...
    public List<PayChannelNew> selectPayChannelNewList(PayChannelNew payChannelNew) {
        ...
        for (PayChannelNew me : list) {
            if (me.getStatus().equals("1")) {
                String redisSuccessRate =  payCacheUtil.getPayChannelSuccessRate(me.getId());
                if (redisSuccessRate!=null){
                    me.setSuccessRate(redisSuccessRate);
                }else {
                    String successRate = payChannelNewMapper.successRate(me.getId());
                    payCacheUtil.setPayChannelSuccessRate(me.getId(),successRate);
