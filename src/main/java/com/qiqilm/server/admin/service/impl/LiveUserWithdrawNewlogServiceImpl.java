...
public class LiveUserWithdrawNewlogServiceImpl implements ILiveUserWithdrawNewlogService {
    ...
    public AjaxResult fixOrder(String[] ids) {
        ...
        for (LiveUserWithdrawNewlog withdrawNewlog : lists) {
            if (lists.get(0).getId().equals(withdrawNewlog.getId())) {
                LiveUserWithdrawNewlog liveUserWithdrawNewlog = new LiveUserWithdrawNewlog();
                liveUserWithdrawNewlog.setWithdrawMoney(sumMoney);
                liveUserWithdrawNewlog.setId(withdrawNewlog.getId());
                ...
                int i = liveUserWithdrawNewlogMapper.updateLiveUserWithdrawNewlog(liveUserWithdrawNewlog);
                log.warn(i + ":合并主播提现订单:" + withdrawNewlog.getId());
            } else {
                LiveUserWithdrawNewlog WithdrawNewlog = new LiveUserWithdrawNewlog();
                WithdrawNewlog.setId(withdrawNewlog.getId());
                WithdrawNewlog.setWstatus(5L);//订单合并已销毁 114.42 120 2.5
                WithdrawNewlog.setRemark("订单合并到：" + lists.get(0).getOrderNo());
                WithdrawNewlog.setUpdateTime(new Date());
                int i = liveUserWithdrawNewlogMapper.updateLiveUserWithdrawNewlog(WithdrawNewlog);
                log.warn(i + ":被合并主播提现订单:" + withdrawNewlog.getId());
            }
        }
