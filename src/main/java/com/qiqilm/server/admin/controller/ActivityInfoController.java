...
public class ActivityInfoController extends BaseController {
    ...
    public AjaxResult getInfo( @PathVariable( "id" ) String id ) {
        ActivityInfo info = activityInfoService.selectActivityInfoById( id );
        if ( StringUtils.isNotBlank( info.getContent() ) ) {
            info.setContent( info.getContent().replaceAll( "\\$\\{domain\\.oss\\}", configDomainCacheUtil.getValue( "domain.oss" ) ) );
        }
    ...
    public AjaxResult add( @RequestBody ActivityInfo activityInfo ) {
        ...
        if ( StringUtils.isNotBlank( activityInfo.getContent() ) ) {
            activityInfo.setContent( activityInfo.getContent()
                                                 .replaceAll( configDomainCacheUtil.getValue( "domain.oss" ), "\\$\\{domain\\.oss\\}" ) );
        }
    ...
    public AjaxResult edit( @RequestBody ActivityInfo activityInfo ) {
        ...
        if ( StringUtils.isNotBlank( activityInfo.getContent() ) ) {
            activityInfo.setContent( activityInfo.getContent()
                                                 .replaceAll( configDomainCacheUtil.getValue( "domain.oss" ),
                                                         "\\$\\{domain\\.oss\\}" ) );
        }
